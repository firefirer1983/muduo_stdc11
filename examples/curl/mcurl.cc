...
int main(int argc, char* argv[])
{
  ...
  curl::Curl m_curl(&loop);

  curl::RequestPtr req = m_curl.getUrl("http://chenshuo.com");
  req->setDataCallback(onData);
  req->setDoneCallback(done);

  curl::RequestPtr req2 = m_curl.getUrl("https://github.com");
  // req2->allowRedirect(5);
  req2->setDataCallback(onData);
  req2->setDoneCallback(done);

  curl::RequestPtr req3 = m_curl.getUrl("http://example.com");
